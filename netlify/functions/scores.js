...
exports.handler = async (event, context) => {
    ...
    try {
        ...
        // NAMA TABEL BARU KHUSUS FILM: movie_scores
        await client.query(`
        ...
        if (event.httpMethod === 'GET') {
            const result = await client.query('SELECT * FROM movie_scores ORDER BY score DESC LIMIT 10');
            return { statusCode: 200, body: JSON.stringify(result.rows) };
        } 
        else if (event.httpMethod === 'POST') {
            const { name, avatar, score, rank } = JSON.parse(event.body);
            await client.query('INSERT INTO movie_scores (name, avatar, score, rank) VALUES ($1, $2, $3, $4)', [name, avatar, score, rank]);
            return { statusCode: 200, body: JSON.stringify({ message: "Skor tersimpan!" }) };
        }
        else if (event.httpMethod === 'DELETE') {
            const { password } = JSON.parse(event.body);
            if (password === '19AG') {
                await client.query('DELETE FROM movie_scores');
